...
export const EditClient = () => {
    ...
    const getClient = useCallback(async () => {
        try {
            const data = await request(`/api/client/${clientId}`, 'GET', null)
            setForm({
    ...
    }, [error, notify, clearError, getClient, form])

    const history = useHistory()

    const createHandler = async () => {
        try {
            const data = await request(`/api/client/${clientId}`, 'PATCH', { ...form })
            notify(data)
                            ...
                            <div className="row mt-3" >
                                <div className="col-md-6" >
                                    ...
                                    >
                                        ...
                                        <option value="2">Kardiolog</option>
                                        <option value="3">Terapevt</option>
                                    </select>
                                </div>
                                <div className="col-md-6">
                                    <label className="labels">
